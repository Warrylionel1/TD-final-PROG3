-- ======================================================
-- SCRIPT D'INSERTION DES DONNÉES DE TEST (JEUDI 06 MAI)
-- BASE DE DONNÉES POSTGRESQL
-- ======================================================

-- Nettoyage préalable (optionnel, mais recommandé)
TRUNCATE TABLE activity_member_attendance, activity_member_concerned, collectivity_activity,
             collectivity_transaction, member_payment, financial_account, membership_fee,
             collectivity_structure, member_referee, collectivity_member, collectivity, member
CASCADE;

-- ------------------------------------------------------
-- 1. MEMBRES (anciens + collectivité 3)
-- ------------------------------------------------------
...
-- Collectivité 1 (et aussi utilisés dans collectivité 2)
...
-- ------------------------------------------------------
-- 5. STRUCTURE DES COLLECTIVITÉS (postes spécifiques)
-- ------------------------------------------------------
...
-- ------------------------------------------------------
-- 6. COMPTES FINANCIERS (anciens + nouveaux pour col-3)
-- ------------------------------------------------------
...
-- Ancien compte col-3 (caisse)
('C3-A-CASH', 'col-3', 'CASH', 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL),
-- Nouveaux comptes col-3 (banque + mobile)
...
-- Les deux tables sont alimentées simultanément.
-- ------------------------------------------------------

-- Pour la collectivité 1 (Tableau 15)
WITH payments_col1 AS (
    SELECT * FROM (VALUES
                  ...
                  ) AS t(id, member_id, fee_id, amount, mode, account_id, date)
)
INSERT INTO member_payment (id, member_id, membership_fee_id, amount, payment_mode, account_credited_id, creation_date)
SELECT id, member_id, fee_id, amount, mode, account_id, date::date FROM payments_col1;
...
SELECT id, 'col-1', date::date, amount, mode, account_id, member_id FROM payments_col1;

-- Pour la collectivité 2 (Tableau 16)
WITH payments_col2 AS (
    SELECT * FROM (VALUES
...
INSERT INTO member_payment (id, member_id, membership_fee_id, amount, payment_mode, account_credited_id, creation_date)
SELECT id, member_id, fee_id, amount, mode, account_id, date::date FROM payments_col2;

INSERT INTO collectivity_transaction (id, collectivity_id, creation_date, amount, payment_mode, account_credited_id, member_debited_id)
SELECT id, 'col-2', date::date, amount, mode, account_id, member_id FROM payments_col2;

-- Pour la collectivité 3 (Tableau 17)
WITH payments_col3 AS (
    SELECT * FROM (VALUES
...
INSERT INTO member_payment (id, member_id, membership_fee_id, amount, payment_mode, account_credited_id, creation_date)
SELECT id, member_id, fee_id, amount, mode, account_id, date::date FROM payments_col3;

INSERT INTO collectivity_transaction (id, collectivity_id, creation_date, amount, payment_mode, account_credited_id, member_debited_id)
SELECT id, 'col-3', date::date, amount, mode, account_id, member_id FROM payments_col3;

-- ------------------------------------------------------
-- 9. NOUVEAUX MEMBRES ADHÉRENTS (Tableaux 18, 19, 20)
-- ------------------------------------------------------
...
-- Liens collectivité 1
INSERT INTO collectivity_member (collectivity_id, member_id) VALUES
                                                                 ('col-1', 'NEW_COL1_M1'), ('col-1', 'NEW_COL1_M2'), ('col-1', 'NEW_COL1_M3'), ('col-1', 'NEW_COL1_M4');

-- Parrainages pour nouveaux membres col-1 (référents C1-M1 et C1-M2)
INSERT INTO member_referee (member_id, referee_id, relation_type) VALUES
